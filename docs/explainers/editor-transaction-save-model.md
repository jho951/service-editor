...
즉 create를 transaction에 포함하는 이유는 "요청 수 감소" 하나가 아니라 다음을 함께 만족하기 위해서다.
...
이 시나리오가 바로 "왜 생성도 transaction 안에 넣는가"를 가장 잘 보여준다.
...
transaction 모델에서는 Enter가 단지 queue 안에 operation 몇 개를 추가하는 편집 이벤트가 된다.
...
3. debounce 만료 시 한 번의 `transactions` 요청으로 저장한다.
...
즉 create와 delete를 transaction에 넣는 이유는 "서버 호출을 줄이기 위해서"만이 아니다.
