...
save는 기존 transaction 기반 설명을 이 기능군 안으로 편입해 상세 기준을 유지하고, move 계열은 초안부터 시작해 점진적으로 보강한다.
...
아래 DTO는 구현 직전 합의용 초안이다.
실제 클래스명은 이 의미를 유지하는 범위에서 조정할 수 있다.
...
public class MoveOperationRequest {
    private MoveResourceType resourceType;
