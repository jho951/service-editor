...
- 요구사항으로 확정된 내용은 최종적으로 `docs/REQUIREMENTS.md`에 반영한다.
- 아직 채택되지 않은 비교나 검토는 `docs/discussions/`에 남긴다.
- 실제로 채택된 결정만 `docs/decisions/` ADR로 승격한다.
- 디버깅/장애 재현 절차는 `docs/runbook/`에 남긴다.
- 개인 학습용 설명과 개인 트러블슈팅 기록은 `docs/learn/`에 남긴다.
- 설명 문서와 구현 가이드는 `docs/explainers/`, `docs/guides/`의 경계를 지킨다.
