...
11. Block 생성 보조 API 확인이 필요하면 `POST /admin/documents/{documentId}/blocks`에 `EditorSaveRequest` 형식의 단건 `BLOCK_CREATE` operation을 보내 `TEXT` 블록을 만든다.
12. Block 정렬 디버깅 시 `sortKey`는 대문자 base36 고정폭 문자열이며, 같은 부모 아래 `ORDER BY sort_key ASC` 결과가 화면 순서와 일치해야 한다.
13. 반복 삽입으로 gap이 없어지면 `SORT_KEY_REBALANCE_REQUIRED(409)`가 반환될 수 있다. 이 경우 즉시 전체 재정렬을 수행하지 않고 후속 reorder/rebalance 작업이 필요하다.
14. Block 생성 실패를 재현할 때는 `parentRef`를 다른 문서의 블록으로 보내거나, 존재하지 않는 `afterRef`를 보내서 `400` 또는 `404` 응답이 요구사항대로 나오는지 확인한다.
15. Block 삭제 보조 API 확인이 필요하면 루트 블록과 하위 블록을 만든 뒤 `DELETE /admin/blocks/{blockId}`에 단건 `BLOCK_DELETE` operation을 보낸다. 대상 블록 트리의 `deletedAt`만 채워지고 같은 문서의 다른 루트 블록은 유지되는지 확인한다.
16. 에디터 표준 저장 경로 확인이 필요하면 `POST /editor-operations/documents/{documentId}/save`에 `BLOCK_CREATE`, `BLOCK_REPLACE_CONTENT`, `BLOCK_MOVE`, `BLOCK_DELETE` batch를 보내고 전체 rollback과 `documentVersion` 갱신 여부를 확인한다.
17. API 통합 테스트는 저장소 루트에서 `./gradlew :documents-boot:test`로 실행한다. 하위 모듈 검증이 필요하면 같은 방식으로 `:documents-api:test`, `:documents-core:test`, `:documents-infrastructure:test`를 선택 실행한다.
18. 빠른 API 계약 확인은 `./gradlew :documents-api:test`, 영속/서비스 구현 확인은 `./gradlew :documents-infrastructure:test`를 우선 사용하고, 최종 조립 확인 시 `:documents-boot:test`를 실행한다.
19. 스키마 명명 규칙 확인이 필요하면 H2 `INFORMATION_SCHEMA.COLUMNS` 또는 MySQL `information_schema.columns`에서 `workspaces.workspace_id`, `documents.document_id`, `blocks.block_id` 컬럼이 생성됐는지 확인한다.
20. 문서 계층 삭제 이슈를 확인할 때는 `information_schema.table_constraints`, `information_schema.referential_constraints`에서 `FK_DOCUMENTS_PARENT`가 존재하는지와 `DELETE_RULE = CASCADE`인지 함께 확인한다.
21. 블록 계층 삭제 이슈를 확인할 때는 `FK_BLOCKS_DOCUMENT`, `FK_BLOCKS_PARENT`가 존재하는지와 두 FK 모두 `DELETE_RULE = CASCADE`인지 함께 확인한다.
22. 문서 hard delete 확인이 필요하면 `DELETE /documents/{documentId}`를 호출한 뒤 `documents`, `blocks` 테이블에서 대상 문서, 하위 문서, 각 문서 소속 블록 row가 실제로 사라졌는지 확인한다.
23. 문서 휴지통 이동 확인이 필요하면 `PATCH /documents/{documentId}/trash`를 호출한 뒤 대상 문서, 하위 문서, 각 문서 소속 블록의 `deleted_at`이 같은 흐름으로 채워졌는지와 대상 문서들의 `version`이 각각 `1` 증가했는지 확인한다.
