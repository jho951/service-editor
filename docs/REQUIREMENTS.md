...
- Gateway는 브라우저 세션이나 외부 Bearer를 내부 서비스에 그대로 전달하지 않고, 검증 성공 뒤 내부 전용 JWT와 사용자 컨텍스트로 정규화해야 한다.
- Gateway는 외부 공개 경로를 `/v1/**`로 제공하고, 내부 서비스 전달 전 경로의 `/v1` 프리픽스를 제거(rewrite)해야 한다.
- 본 서비스는 Gateway가 주입한 `X-User-Id`와 내부 `Authorization`을 인증 컨텍스트로 사용한다.
- 기본 내부 JWT 검증 계약은 `issuer=api-gateway`, `audience=internal-services`이며, shared secret은 Gateway와 동일해야 한다.
