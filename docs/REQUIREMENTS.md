...
- 같은 워크스페이스 안에서는 활성 문서와 휴지통 문서를 포함해 `title`이 중복되지 않아야 한다.
...
- 문서 제목 수정 결과가 같은 워크스페이스의 다른 문서 제목과 중복되면 `409 Conflict`를 반환해야 한다.
...
- 복구 시 같은 워크스페이스의 다른 문서 제목과 중복되면 복구를 실패 처리해야 한다.
...
## 9.3 문서 검증
- `title` 최대 길이: `255`
- `title`은 같은 워크스페이스 안에서 활성 문서와 휴지통 문서를 포함해 유일해야 한다.
