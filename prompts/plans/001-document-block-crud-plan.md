...
4. 인프라 계층 설계
   `documents-infrastructure`에 MyBatis mapper, XML, 영속 모델 매핑, 트랜잭션 경계를 정의한다.
5. API 계약 설계
...
7. 검증 계획 수립
   서비스 단위 테스트, mapper 연동 검증, 컨트롤러 요청/응답 검증, 기본 디버깅 절차를 정리한다.
8. 문서 동기화
...
- 리스크: 문서 저장 단위를 문서 전체 JSON으로 둘지 블록 단위 레코드로 둘지에 따라 API와 MyBatis 쿼리가 크게 달라진다.
