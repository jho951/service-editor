...
class AdminBlockControllerWebMvcTest {
    ...
    @Mock
    private AdminBlockOperationService adminBlockOperationService;
    ...
    @Test
    @DisplayName("성공_생성 API는 editor save create 요청을 그대로 위임한다")
    void createBlockDelegatesSingleCreateSave() throws Exception {
        UUID documentId = UUID.randomUUID();
        when(adminBlockOperationService.applyCreate(eq(documentId), any(), any(), eq("user-123")))
                .thenReturn(saveResult(documentId, "tmp:block:1", UUID.randomUUID(), 1L, "000000000001000000000000", null));
        ...
        verify(adminBlockOperationService).applyCreate(eq(documentId), any(), any(), eq("user-123"));
    }

    @Test
    @DisplayName("성공_수정 API는 blockId로 문서 ID를 찾아 editor save 경로를 호출한다")
    void updateBlockDelegatesSingleReplaceContentSave() throws Exception {
        UUID blockId = UUID.randomUUID();
        UUID documentId = UUID.randomUUID();
        when(adminBlockOperationService.applyReplaceContent(eq(blockId), any(), any(), eq("user-456")))
        ...
        verify(adminBlockOperationService).applyReplaceContent(eq(blockId), any(), any(), eq("user-456"));
    }

    @Test
    @DisplayName("성공_삭제 API는 editor save delete 응답을 반환한다")
    void deleteBlockDelegatesSingleDeleteSave() throws Exception {
        UUID documentId = UUID.randomUUID();
        UUID blockId = UUID.randomUUID();
        LocalDateTime deletedAt = LocalDateTime.of(2026, 3, 25, 12, 0);
        when(adminBlockOperationService.applyDelete(eq(blockId), any(), any(), eq("user-123")))
                .thenReturn(saveResult(documentId, null, blockId, null, null, deletedAt));
        ...
        verify(adminBlockOperationService).applyDelete(eq(blockId), any(), any(), eq("user-123"));
    }

    @Test
    @DisplayName("성공_이동 API는 editor save move 응답을 반환한다")
    void moveBlockDelegatesSingleMoveSave() throws Exception {
        UUID blockId = UUID.randomUUID();
        UUID documentId = UUID.randomUUID();
        when(adminBlockOperationService.applyMove(eq(blockId), any(), any(), eq("user-123")))
                .thenReturn(saveResult(documentId, null, blockId, 2L, "000000000002000000000000", null));
        ...
        verify(adminBlockOperationService).applyMove(eq(blockId), any(), any(), eq("user-123"));
    }
    ...
    private EditorSaveResult saveResult(
            UUID documentId,
            String tempId,
            UUID blockId,
            Long version,
