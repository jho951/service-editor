...
- 에이전트는 일반론, 익숙한 패턴, 통상적 베스트 프랙티스보다 먼저 전역 규칙, 현재 모듈/패키지 구조, 기존 코드 경계를 따른다. 로컬 규칙과 충돌하는 일반론을 임의로 우선 적용하지 않는다.
...
- 불확실한 사실은 추정해서 단정하지 않는다. 코드, 문서, 요구사항만으로 바로 확정할 수 없으면 모른다고 명시하고 먼저 확인한다.
- 사용자 지시나 요구사항 해석이 둘 이상 가능하면, 구현 전에 해석이 필요한 지점을 먼저 짚고 사용자에게 확인한다. 에이전트가 임의로 더 나은 방향이라고 판단해 외부 계약이나 기존 동작을 바꾸지 않는다.
