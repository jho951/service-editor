...
- 구현 시 가독성(명시성)과 성능 사이에 큰 차이가 없다면 가독성(명시성)을 우선합니다.
- 메서드 분리는 역할 경계가 더 선명해질 때만 적용합니다. 성능 차이가 거의 없고 흐름 파악만 어려워진다면 과도한 helper/overload 분할보다 한 번에 읽히는 구현을 우선합니다.
- 작은 처리 단위가 바뀌는 지점에서는 빈 줄과 들여쓰기를 사용해 읽기 단위를 드러냅니다. 문장이 바뀌었는데도 선언과 호출을 과하게 밀집시키는 형식은 피합니다.
- 메서드명은 축약보다 역할과 결과가 바로 드러나는 쪽을 우선합니다. 호출부만 봐도 무엇을 수집/검증/변경하는지 추측이 아니라 해석이 가능해야 합니다.
